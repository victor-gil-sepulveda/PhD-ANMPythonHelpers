...
class BaharServerFilesParser(object):
    ...
    def read(cls, eigenvalues_file_path, eigenvectors_file_path):
        ...
        if eigenvalues_file_path is not None and eigenvalues_file_path != "":
            eigenvalues = cls.read_eigenvalues(eigenvalues_file_path)
        
        return (eigenvalues, cls.read_eigenvectors(eigenvectors_file_path))
        
    @classmethod
    def read_eigenvalues(cls,  eigenvalues_file_path):
        """
    ...
    @classmethod  
    def read_eigenvectors(cls, eigenvectors_file_path):
        """
        Reads a file containing eigenvectors (20) written in columns.
        """ 
        return numpy.loadtxt(eigenvectors_file_path).T[1:-1]
