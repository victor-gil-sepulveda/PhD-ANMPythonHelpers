...
def treat_prolines_in_eivgenvectors(master_seq, old_eigenvectors):
    """
    ...
    for i, res in enumerate(master_seq):
        ...
        if res == "PRO":
            #only psi
            new_eigenvectors.append(old_eigenvectors[(i*2) + 1])
        else:
            # phi
            new_eigenvectors.append(old_eigenvectors[i*2])
            # psi
            new_eigenvectors.append(old_eigenvectors[(i*2) +1])
...
if __name__ == '__main__':
    ...
    pro_eigenvectors = treat_prolines_in_eivgenvectors(sequences[0], eigenvectors)
